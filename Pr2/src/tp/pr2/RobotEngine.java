...
import java.util.Scanner;	//TODO Falta terminar esta clase, y hacer los use de cada objeto.
	...
	// RobotEngine: Consta del lugar actual del robot, la direcci贸n en la que mira, y el mapa por el que se mueve (array de calles).
public class RobotEngine {
	...
	//Duevuelve la calle hacia la que est谩 mirando wall路e, null si no hay calle.
		...
		while (!initialPlace.isSpaceship() && !quit && (this.fuel > 0)) {
			...
			if (!instruction.isValid())	// Comprueba si la instrucci贸n introducida la reconoc walle. Si no, muestra el mensaje corespondiente.
				...
				switch(instruction.getAction()){
					...
					default:  //case TURN: Como la instrucci贸n es correcta, o gira a la derecha o a la izquierda.
					{	
						if (instruction.getRotation().equals(Rotation.RIGHT))
							direction = direction.turnRight();
						else
							direction = direction.turnLeft();
						lookingDirection(this.direction);	// Despu茅s de haber girado, actualiza la direcci贸n en la que est谩 mirando.
					}
				}
			}
		}
		sc.close();	// Cierra el esc谩ner

		if (!quit)	// Si no se ha elegido la opci贸n quit, es que se ha llegado a la nave. Se muestra el mensaje correspondiente.
			say("I am at my spaceship. Shutting down... Bye bye");
		else
			say("I have communication problems. Bye bye");	// Se ha elegido la opci贸n quit, luego se m
	}
	/* comprueba si desde el lugar en el que est谩 wall路e, hacia donde est谩 mirando hay una calle. 
	Para ello, recorre el array de calles, comprobando con el m茅todo comeOutFrom.
	En caso de que walle est茅 mirando hacia una calle, le avanza hasta el lugar que hay al otro lado de la calle con el nextPlace y 
